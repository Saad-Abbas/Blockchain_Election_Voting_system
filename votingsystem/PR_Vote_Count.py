...
def vote_Count(election_id,Admin_ID): #-----------insert Admin_Id parameter
    ...
    E_Record = db_election.find_one({'_id': ObjectId(election_id), 'Admin_ID': ObjectId(Admin_ID) }) #insert AdminID Parameter here
    if E_Record != None:
        # print(E_Record)
...
def Total_party_vote_Count(Admin_ID,election_id):
    ...
    for party in party_list:
        ...
        count = db_ballot.count_documents({'Admin_ID': str(Admin_ID) ,'Election_name' : Election_Name['Election_Name'],'Vote': party    })  ##,'Election_name': Election_Name['Election_Name']
            # print(party + ' '+ str(count))        
        party_votes[party] = count 
        count = 0 
        Wining_Party =max(party_votes,key=party_votes.get)
    dispaly(party_votes)  
    print('dhkfgdghhhhhhhh',party_votes)     
...
def Vote_Count(state_name,Admin_ID): #Admin_ID,Election_Name
    ...
    for party in party_list:
        count = db_ballot.count_documents({'Admin_ID': str(Admin_ID),'Vote': party ,'State_Name':state_name,'Election_name' : E_Record['Election_Name'] }) #ObjectId(AdminID) , Election_Name,State_Name
        vote[party]= count

    find_state_name = db_States.find_one({'State_name' : state_name})
    E_vote = find_state_name['Electoral_Votes']

    value = check_Max_value(vote)
    check = check_party_votes(value,vote)

    if check == 1:

        wining_party = Who_Win(vote)
        vote['Election_Status'] = 'Winner Decleared'
        vote['State Name'] = state_name
        vote['Electoral Vote'] = E_vote
        vote['Wining_Party'] = wining_party
        vote['Assign ' + str(state_name) + ' Electoral Vote to ' + str(wining_party)] = E_vote

        dispaly(vote)
        return vote

    else:
        vote['Election_Status'] = 'Draw'
        vote['State_Name'] = state_name
        vote['Electoral Vote'] = E_vote
        vote['Assign ' + str(state_name) + ' Electoral Vote to Nobody'] = 0
        dispaly(vote)
        return vote
     

def check_party_votes(check,vote): #check Count
    count = 0
    for key,Value in vote.items():
        if check == Value:
            count += 1
...
def check_Max_value(Vote):  #Extract Maximum Value 
    All_value = Vote.values()
...
#     else:
#         vote['Election_Status'] = 'Draw'
#         vote['State_Name'] = state_name # State_list[s]
#         vote['E_Vote']= find_state_name['Electoral_Votes']
#         dispaly(vote)
#         return vote
